...
import 'package:sevenapplication/core/services/database/service_services.dart';
...
class WorkerProfil extends StatefulWidget {
  ...
  @override
  _WorkerProfilState createState() => _WorkerProfilState();
}
...
class _WorkerProfilState extends State<WorkerProfil> {
  ...
  late Service initialUserService;
  late String selectedspecialite = '';
  ...
  Future<void> initSpecialites() async {
    ...
    String userservice = widget.utilisateur!.get('service').objectId;
    final user_service = await ServiceServices.getServiceFromID(userservice);

    if (user_service.name != null) {
      initialUserService = user_service;
      setState(() {
        selectedspecialite = user_service.name!;
      });
    }
    setState(() {
  ...
  Future<void> submitProfilHandler() async {
    ...
    Service userservice =
        await ServiceServices.getServiceFromName(selectedspecialite);
    ...
    await updateUser2(nomuser, emailuser, passworduser, phonenumero,
        userservice.objectId, null);
    _refreshPage();
  }
  ...
  Future pickImage() async {
    ...
    /* try {
      ...
      print("type of imageTemp : ${imageTemp.runtimeType}");
      //await addUserAvatar(imageTemp);
  ...
  Future pickImageC() async {
    ...
    /* try {
      ...
      /* setState(() => this.imageactu = imageTemp); */
      await addUserAvatar(imageTemp);
    } on PlatformException catch (e) {
      print('Failed to pick image: $e');
    } */
  ...
  Future<void> CheckUserInfoChanges(change) async {
    if (change == 1) {
      ...
      final selectedservice =
          _formKey_profil.currentState!.fields['specialite']!.value;
      if (initialUserService.name == selectedservice) {
        setState(() {
          isButtonClickable = false;
        });
      }
                                  ...
                                  children: [
                                    Container(
                                      ...
                                      child: FormBuilderDropdown(
                                        name: 'specialite',
                                        initialValue: selectedspecialite,
                                  ...
                                  child: Column(
                                    children: [
                                      ...
                                      FormBuilderTextField(
                                        ...
                                        decoration: InputDecoration(
                                          icon: const Icon(Icons.lock),
                                          iconColor: passwordconfirmhaserror
                                              ? Colors.red
                                              : Colors.black,
                                          labelText:
                                              "Confirmez le mot de passe *",
                                          labelStyle: TextStyle(
                                            color: passwordconfirmhaserror
                                                ? Colors.red
                                                : Colors.black,
                                          ),
                                          border: OutlineInputBorder(
